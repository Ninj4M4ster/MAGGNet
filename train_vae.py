...
DATA_DIR = "/app/output/RPI/"

OUTPUT_PATH = "/app/output/maggnet/experiment_test"
...
TEST_FOLDERS = ['body2', 'body3', 'body4', 'body5', 'body6', 'body7', 'body8', 'body9', 'body10', 'body11', 'body12', 'body13', 'head1', 'head2']
...
def read_raw_image(path, target_size=(512, 512), normalize=True):
...
def save_raw_image(tensor, path):
    ...
    img_np = (img_np * 0.5) + 0.5 # Denormalizacja
...
def collect_training_files(base_dir, test_folders_list):
    ...
    for folder_name in subfolders:
        ...
        dir_free = os.path.join(folder_full_path, "clean")
        dir_art = os.path.join(folder_full_path, "artifact")
        
        if not (os.path.exists(dir_free) and os.path.exists(dir_art)):
            continue

        f_free = sorted(glob.glob(os.path.join(dir_free, "*.png")))
        f_art = sorted(glob.glob(os.path.join(dir_art, "*.png")))
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        # y_np = read_raw_image(self.target_files[idx])
        # x_a_np = read_raw_image(self.art_files[idx])

        y = torch.from_numpy(np.array(Image.open(self.target_files[idx]).convert('L').resize((512,512)))/127.5-1).float().view(1,512,512)
        x_a = torch.from_numpy(np.array(Image.open(self.art_files[idx]).convert('L').resize((512,512)))/127.5-1).float().view(1,512,512)
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        for y, x_a in loop:
            ...
            total_G = loss_GAN + loss_content + loss_edge + loss_bg + (F.mse_loss(fake_final * m, m * 1.0) * LAMBDA_METAL) + loss_KLD
            total_G.backward()
            opt_G.step()
            ...
            if i % 100 == 0:
                # Use only the first sample in the batch for preview
                preview = torch.cat([y[0:1], g[0:1]*2-1, x_a[0:1], fake_final[0:1], content_mask[0:1]], dim=3)
