...
TEST_DIR = "/app/data/RPI/Body/body5"
OUTPUT_CSV = "/app/output/experiments_output/evaluation_results.csv"

# standard or vae
MODEL_TYPE = "vae" 
# raw or png
CHECKPOINT_PATH = "/app/output/experiments_output/checkpoints/adn_generator_epoch99.pth"
...
def get_guidance_from_mask(mask_np):
    h, w = mask_np.shape
    guidance = np.zeros((h, w), dtype=np.float32)
    if np.sum(mask_np) > 0:
        guidance = cv2.GaussianBlur(mask_np, (21, 21), 10)
        guidance = guidance / (guidance.max() + 1e-8)
    return guidance
...
def evaluate():
    ...
    # 3. Zbieranie plików
    dir_target = os.path.join(TEST_DIR, "Target")
    dir_art = os.path.join(TEST_DIR, "Baseline")
    ...
    min_len = min(len(f_targets), len(f_arts), len(f_masks))
    ...
    with torch.no_grad():
        for i in tqdm(range(min_len), desc="Przetwarzanie"):
            ...
            if style_metric is not None:
                val_style = style_metric(gen_01, real_01)
